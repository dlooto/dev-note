$((function() {
	$.extend(String.prototype, {
		toHTML: function(f) {
			var r = this.replace(/\<script/g, '<******').replace(/\&lt;script/g, '<******').replace(/&/g, "&amp;").replace(/\"/g, "&quot;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/ /g, "&nbsp;").replace(/\n/g, '<br>').replace(/\s|\t/g, ' ');
			return r;
		}
	});
	...
	var init = function() {
		...
		window.loginWin.alert = function(s, close_callback) {
			...
			(typeof(s && s._show) === "undefined") && (o.text = "<br/>" + o.text + "<center><input type=\"button\" class=\"devSubmit closeBtn\" value=\""+o.ok_text+"\"/></center>");
...
function checkuserLogin(url) {
	...
	regweibo = window.regweibo || '0';
	url = url || encodeURIComponent(location.href);
	if (hdlogin && regweibo == '1') {
		return true;
	} else if (hdlogin == undefined || hdlogin == '' || hdlogin == '0') {
		showLoginWin(url);
		return false;
	} else {
		loginWin.alert('<center>您还未注册腾讯微博，<a href="http://t.qq.com" target="_blank">请先注册</a>。</center>');
	}
	return false;
}

function openAppAfterLogin(o) {
...
var common = {
	getMsgByRet :function(i){
		return {
			 '-201':'对不起，请求参数错误！'
			,'-202':'请求被拒绝，请稍后再试！'
			,'-400':'登录信息已丢失，请 <a href="javascript:;" onclick="showLoginWin()">重新登录</a> ！'
...
function ptlogin2_onResize(w, h){console.log([w,h].toString());
	$(".modulebox_content").removeAttr("style").height(h+40).parent().height(h+75).css("margin-top",-Math.ceil((h+75)/2));
	$("#login_div").height(h);
}/*  |xGv00|874d8234c267854fd8266d93056c2ec0 */